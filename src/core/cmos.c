...
#include <musl/include/stdbool.h>
...
cmos_rtc_t cmos_read_rtc()
{
    ...
    do {
        ...
        // Something's wrong when using musl libc's memcpy
        // memcpy(&rtc, &last, sizeof(cmos_rtc_t));
        last.seconds = rtc.seconds;
        last.minutes = rtc.minutes;
        last.hours = rtc.hours;
        last.weekdays = rtc.weekdays;
        last.day = rtc.day;
        last.month = rtc.month;
        last.year = rtc.year;
        last.century = rtc.century;
